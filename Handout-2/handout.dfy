class Map {
    ...
    ensures !z ==> keyNotFound(k)
  {
   ...
   method put(k:int, v:char)
    requires absInv()
    //WARNING: this method couldn't be fully implemented 
    //since dafny couldn't understand the update of the ghost variable
    ensures repInv()
    modifies a, `a, `m
  {
    ...
    if( !f )
    {
      ...
      var len:int := |a[pos]|;
      assert forall i :: 0 <= i < len-1 ==> a[pos][i] == old(a[pos][i]);
